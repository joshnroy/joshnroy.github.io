...
- **CV**: the PDF lives at `assets/pdf/Josh_Roy_CV.pdf`. `_pages/cv.html` is a bare page that instantly redirects (meta refresh 0 + JS) `/cv/` to the PDF — there is no embed page. The CV is reachable only via the navbar page — intentionally not in the social icon rows. (The al-folio structured-CV renderer is intentionally not used.)
